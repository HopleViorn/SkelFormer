...
class VectsetVAE(nn.Module):
    ...
    def latents2mesh(self, latents: torch.FloatTensor, **kwargs):
        ...
        # Visualize grid_logits
        if grid_logits is not None:
            grid_logits_np = grid_logits.squeeze().cpu().numpy()
            if grid_logits_np.ndim == 3:
                # Plot slices of the 3D grid
                fig, axes = plt.subplots(1, 3, figsize=(15, 5))
                
                # Slice along Z axis
                z_slice = grid_logits_np.shape[2] // 2
                z_slice = grid_logits_np.shape[2] // 2
                im = axes[0].imshow(grid_logits_np[:, :, z_slice], cmap='rainbow')
                axes[0].set_title(f'Slice Z={z_slice}')
                axes[0].set_xlabel('X')
                axes[0].set_ylabel('Y')
                fig.colorbar(im, ax=axes[0])
                
                # Slice along Y axis
                y_slice = grid_logits_np.shape[1] // 2
                im = axes[1].imshow(grid_logits_np[:, y_slice, :], cmap='rainbow')
                axes[1].set_title(f'Slice Y={y_slice}')
                axes[1].set_xlabel('X')
                axes[1].set_ylabel('Z')
                fig.colorbar(im, ax=axes[1])
                
                # Slice along X axis
                x_slice = grid_logits_np.shape[0] // 2
                im = axes[2].imshow(grid_logits_np[x_slice, :, :], cmap='rainbow')
                axes[2].set_title(f'Slice X={x_slice}')
                axes[2].set_xlabel('Y')
                axes[2].set_ylabel('Z')
                fig.colorbar(im, ax=axes[2])
                
                plt.tight_layout()
                plt.savefig("grid_logits_visualization.png")
                plt.close(fig)
                logger.info("Saved grid_logits visualization to grid_logits_visualization.png")
